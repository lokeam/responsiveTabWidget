	...
	var lineupwWidget = {
		...
		ajax : function() {
			...
			if( self.params.useProxy ){
				endpoint = [ proxyurl, '?callback=lineupCallback&url=', endpoint ].join( '' );
			}else{
		...
		makeIndexedObjects: function( json, settings ){
			var self = this,
			...
			self.onWindowLoadEvent(artistsWithVideos, embedSettings);
			return composite;
		},
		processJSON : function(json, settings){
			...
			self.compositeJson = self.makeIndexedObjects( json, settings );
			//console.log( self.compositeJson );
		...
		buildRootTemplate : function(allRender) {
			...
			for (var i = 0; i < whlen; i++ ) {
				rootView += " <li class='tab'> <a href='#tab-"+i+"' class='active'> <span>"+allRender.when.rootTemplate.days[i]+"</span> <div class='dayofmonth'>"+allRender.when.rootTemplate.daysOfMonth[i]+"</div> </a> </li>";
			}
			rootView += "</ul><div class='panel-container'>";

			for (var i = 0; i < whlen; i++) {
				rootView += "<div id='tab-"+i+"'>";
				if ( (allRender.settings.buttonText !== '') && (allRender.settings.buttonURL !== '') ) {
			...
			for ( var i = 0; i < wlen; i++ ) {
				rootView += "<li class='tab'><a href='#tab-"+i+"'>"+allRender.where.rootTemplate[i]+"</a></li>";
			}
					...
					if ( allRenderVar[i].artistObjLi === "headline") {
						...
						if( self.compositeJson.isSchedule ){
							...
							for ( var j = 0, slen = showtimes.length; j < slen; j++ ) {
								artistsTab += "<div class='artistInfo'><p>"+showtimes[j].formattedStartMonth+" "+showtimes[j].startDateDayOnly+"</p><p><span class='artistTime'>" + ( showtimes[j].formattedTimeStart?showtimes[j].formattedTimeStart:"" ) + "</span> " + ( self.compositeJson.isVenue?"| <span class='artistPlace'>"+showtimes[j].venueId+"</span>" : "" ) + "</p></div>"
							}
							artistsTab += "</div></div>";
						}
						...
						artistsTab += "</div><div id='artist-tab-d' class='globalshare'><div class='artistWrapper'><h2>Share</h2><div class='artistShare mediaObj'> <a target='_blank' data-shareimage='" + self.cleanImageUrl( allRenderVar[i].imageURL ) + "' href='"+allRenderVar[i].facebook+"' class=''> <div class='siconcircle fb media'> <i class='fa fa-facebook-f'></i></div><p>share on facebook</p></a> </div><div class='artistShare mediaObj'><a target='_blank' href='"+allRenderVar[i].twitter+"' data-festivalname='"+self.compositeJson.festivalName+"' data-artistname='"+allRenderVar[i].title+"' class=''><div class='siconcircle tw media'> <i class='fa fa-twitter'></i> </div><p>share on twitter</p></a></div><div class='artistShare mediaObj'> </div></div></div></div></div><footer class='socialfooter'> <div class='socialIcons'><a target='_blank' href='"+allRenderVar[i].web+"'><div class='siconcircle ws "+allRenderVar[i].wsToggle+"'> <i class='fa fa-bullseye'></i> </div></a><a target='_blank' href='"+allRenderVar[i].facebook+"'><div class='siconcircle fb "+allRenderVar[i].fbToggle+"'> <i class='fa fa-facebook-f'></i> </div></a><a target='_blank' href='"+allRenderVar[i].youtube_channel+"'><div class='siconcircle yt "+allRenderVar[i].ycToggle+"'> <i class='fa fa-youtube'></i> </div></a><a target='_blank' href='"+allRenderVar[i].twitter+"'><div class='siconcircle tw "+allRenderVar[i].twToggle+"'> <i class='fa fa-twitter'></i> </div></a><a target='_blank' href='"+allRenderVar[i].instagram+"'><div class='siconcircle ig "+allRenderVar[i].inToggle+"'> <i class='fa fa-instagram'></i> </div></a></div></footer></ul>";
						/*testing inline version of artist mod */
						...
						if ( lastFeatured) {
                            ...
                            if ( tabName == 'artists' ) {
                            	artistsTab += "<li class='artistSlot "+allRenderVar[i].artistObjLi+"'><a class='"+allRenderVar[i].artistObjA+"' href=''><img class='"+allRenderVar[i].artistObjImg+"' src='" + blankImg + "' data-echo='"+self.cleanImageUrl( allRenderVar[i].imageURL )+nonFeaturedArtistParams+"'><div class='artistInfo'><p>"+allRenderVar[i].title+"</p><p>More info</p></div></a>";
                            }
							...
							artistsTab += "</div><div id='artist-tab-d' class='globalshare'><div class='artistWrapper'><h2>Share</h2><div class='artistShare mediaObj'> <a target='_blank' data-shareimage='"+self.cleanImageUrl( allRenderVar[i].imageURL )+"' href='"+allRenderVar[i].facebook+"' class=''> <div class='siconcircle fb media'> <i class='fa fa-facebook-f'></i></div><p>share on facebook</p></a> </div><div class='artistShare mediaObj'><a target='_blank' href='"+allRenderVar[i].twitter+"' data-festivalname='"+self.compositeJson.festivalName+"' data-artistname='"+allRenderVar[i].title+"' class=''><div class='siconcircle tw media'> <i class='fa fa-twitter'></i> </div><p>share on twitter</p></a></div><div class='artistShare mediaObj'> </div></div></div></div></div><footer class='socialfooter'><div class='socialIcons'><a target='_blank' href='"+allRenderVar[i].web+"'><div class='siconcircle ws "+allRenderVar[i].wsToggle+"'> <i class='fa fa-bullseye'></i> </div></a><a target='_blank' target='_blank' href='"+allRenderVar[i].facebook+"'><div class='siconcircle fb "+allRenderVar[i].fbToggle+"'> <i class='fa fa-facebook-f'></i> </div></a><a target='_blank' href='"+allRenderVar[i].youtube_channel+"'><div class='siconcircle yt "+allRenderVar[i].ycToggle+"'> <i class='fa fa-youtube'></i> </div></a><a target='_blank' href='"+allRenderVar[i].twitter+"'><div class='siconcircle tw "+allRenderVar[i].twToggle+"'> <i class='fa fa-twitter'></i> </div></a><a target='_blank' href='"+allRenderVar[i].instagram+"'><div class='siconcircle ig "+allRenderVar[i].inToggle+"'> <i class='fa fa-instagram'></i> </div></a></div></footer></ul>";
							artistsTab += "</li>";
						}
				...
				if ( allRender.when ) {
					for ( var i = 0, sl = showDays.length; i < sl; i++ ) {
						var key = showDays[i],
							...
							toInject = $('#nested-tab-2 .panel-container #tab-'+i+' .dl-menu');
				...
				if ( allRender.where ) {
					for ( var i = 0, sl = showVenues.length; i < sl; i++ ) {
                        var key = showVenues[i],
                            ...
                            toInject = $('#nested-tab-3 .panel-container #tab-'+i+' .dl-menu');
		...
		onWindowLoadEvent : function(vidlisting, embedSettings) {
			var self = this,
				vidlisting = vidlisting;
			$(window).load(function() {
				self.setEmbedStyles(embedSettings);
                self.onNavTabClickEvent(vidlisting);
				self.initTabs();
				self.initFlyin();
				//console.log('removing overlay');
				$('#overlay').remove();
                //console.log('on window load event set');
			});
		},
		setEmbedStyles: function(embedSettings) {
			...
			var self = this;
			var customStyles = 
			"<style id='embedCodeStyles' type='text/css'>"+
			".tab-container {background-color: "+embedSettings.bodyBGColor+"}"+
			".artists .artistSlot .artistInfo, .artists .artistSlot .artistInfo p, .customArtistButtonTextRoot, .customArtistButtonText, .artistShare a p, .dl-submenu li.dl-back > a, .widgetMessage, .dl-submenu .artistInfo p span.artistPlace, .dl-submenu .artistInfo p span.artistPlace {color: "+embedSettings.artistCopyColor+"}"+
