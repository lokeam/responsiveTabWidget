( function( $, window, undefined ) {
  ...
  $.DLMenu.prototype = {
	...
	_config : function() {
	  ...
	  this.$el.find( 'ul.dl-submenu' ).prepend( '<li class="dl-back top"><i class="fa fa-angle-left fa-2x"></i><a id="dl-back-a" href="#">' + this.options.backLabel + '</a></li>' );
	  this.$back = this.$menu.find( 'li.dl-back' );
	  // Set the label text for the back link.
	  if (this.options.useActiveItemAsBackLabel) {
		this.$back.each(function() {
		  var $this = $(this),
			parentLabel = $this.parents('li:first').find('a:first').find('.artistInfo').find('p:first').text();
    ...
    if( !$( event.currentTarget ).hasClass( 'dl-subviewopen' ) ){
  		var
        elem = window.top.document.querySelector( '#lineupwidget-iframe' ),
  		  rect = elem.getBoundingClientRect();
  		
  		window.top.scrollTo( 0, window.top.document.body.scrollTop+rect.top );
    }
		var $item = $(this),
		  ...
		  $customButton = $('.customArtistButtonTextRoot'),
		  ...
		  /*var elem = window.top.document.querySelector('#lineupwidget-iframe');
		...
		if( ($submenu.length > 0) && !($(event.currentTarget).hasClass('dl-subviewopen'))) {
		  ...
		  //var $flyin = $everything.clone().css( 'opacity', 0 ).insertAfter( $menutop ),
			onAnimationEndFn = function() {
			  //console.log('inside flyin fn, ln 148');
