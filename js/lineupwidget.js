		...
		processJSON : function(json){
			...
			var _convertDate = function(shows, sIndex) {
				...
				if (formattedStartDay == formattedEndDay) {
					formattedSingleDayShow = formattedStartDay;
					shows[sIndex].formattedSingleDayShow = formattedSingleDayShow;
				}
			...
			var prepArtistTab = function(venues, shows, artists) {
				...
				for ( var i = 0, alen = artists.length; i < alen; i++ ) {
					...
					if ( artists[i].multiShowArtist ) {

						artists[i].venues.sort(function(a, b) {
			...
			var prepWhenTab = function() {
				...
				// find dupes in showsByDate
				// 1. find duplicate artist
				// 2. add duplicate artist flag to artist object
				// 3. in rendering fn, set duplicate artist conditional:
				//	if second artist is a duplicate, grab venue and add as another property
				//	if following artist is also a duplicate do the same
				// 4. add conditional in render function: if artist is duplicate, add venues from array
				...
				for (var i in showtimeComposite.showsByDate ) {
					...
					for (var j = 0, slen = showtimeComposite.showsByDate[i].length; j < slen; j++ ) {
						for ( var k = j + 1; k < slen; k++ ) {
							...
							if ( showtimeComposite.showsByDate[i][j].id === showtimeComposite.showsByDate[i][k].id ) {
								duplicateFound = true;

								showtimeComposite.showsByDate[i][j].multiShowArtist = true;
								dupArr.push( trim(showtimeComposite.showsByDate[i], "id") );
							}
						}
					}
					if ( duplicateFound ) {
						showtimeComposite.showsByDate[i] = dupArr[0];
					}
				}
				...
				for (var i = 0, alen = allRenderVar.length; i < alen; i++) {
					...
					if ( allRenderVar[i].artistObjLi === "headline") {
						if ( tabName == 'when' ) {
							...
							if ( allRenderVar[i].venues[multiShowArtistWhen[ artistId ]].formattedSingleDayShow ) {
							...
							if (allRenderVar[i].multiShowArtist) {
								for ( var j = 0, vlen = allRender[i].venues.length; j < vlen; j++ ) {
									artistsTab += "<span class='artistPlace'>" + allRenderVar[i].venues[j].venueId + "</span>";
								}
								artistsTab += "</p></div></a>";
							} else {
								artistsTab += "<span class='artistPlace'>" + artistVenue + "</span></p></div></a>";
							}
						...
						if ( tabName == 'artists' ) {
							...
							if ( allRenderVar[i].venues[multiShowArtistWhen[ artistId ]].formattedSingleDayShow ) {
								artistsTab += "<li id='"+allRenderVar[i].id+"' class='artistSlot "+allRenderVar[i].artistObjLi+"'><a class='"+allRenderVar[i].artistObjA+"' href='javascript:'><img class='"+allRenderVar[i].artistObjImg+"' src='" + blankImg + "' data-echo='" + self.cleanImageUrl( allRenderVar[i].imageURL ) + ""+nonFeaturedArtistParams+"'><div class='artistInfo'><p class='artist-title'>"+allRenderVar[i].title+"<span class='artist-subtitle'>" + subtitle + "</span></p><p><span class='artistTime'>" + artistTime + "</span> <span class='artistPlace'>"+( self.compositeJson.isVenue ? allRenderVar[i].venues[0].venueId : '' )+"</span></p></div></a>";
							} else {
